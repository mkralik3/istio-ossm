...
// For OSSM3.3+ on FIPS, Proxy validating all keys before applying config. It can take longer for certain situations, e.g.:
// When DestinationRule for the whole mesh is applied, the proxy can validate 400+ keys, which can take ~1minute. Tests need to wait till that situation, so
// after each creation/deletion of DestinationRule with could change `tls` mode, we need to wait till proxy is updated. 
// istio-ingress proxy takes the longest according to observation.
...
// if not (timeouted after 1 minute), the proxy is considered as notReady/updating proxy config. It tries 4 times (4minutes max)
		...
		for _, pod := range pods.Items {
			...
			for attempt := 1; attempt <= maxRetries; attempt++ {
				...
				if attempt == maxRetries {
					scopes.Framework.Errorf("failed getting proxy config for pod %s/%s after %d attempts: %v. The proxy is considered as not ready.", pod.Namespace, pod.Name, maxRetries, err)
				} else {
